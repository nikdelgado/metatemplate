...
def _is_native(type_name: str) -> bool:
    py_lookup = locate(type_name)
    return bool((type_name in YAML_TYPE_MAP and type_name not in CUSTOM_TYPES) or ( py_lookup and type_name in __builtins__ ))
...
def _build_struct(name: str, data: Dict) -> Class:
    ...
    if extension := data.pop('extends', data.pop('parent', None)):
        if '.' in extension or '::' in extension:
            raise RuntimeError(f'Class {name} contains external parent {extension}, unsupported feature due to constructor complexities.')
        target.extensions = [
            Extension(
                type=AttrType(
                    qname=_lookup_qname(extension),
                    native=False
                ),
                tag=extension,
    ...
    for field in data.get('fields', []):
        ...
        attr_type = AttrType(
            qname=_lookup_qname(field_type),
            native=_is_native(field_type)
        )
...
def _build_enum(name: str, data: Dict) -> Class:
    ...
    attr_type = AttrType(
        qname=_lookup_qname(enum_base_type),
        native=_is_native(enum_base_type)
    )
...
def _build_variant(name: str, data: Dict) -> Class:
    ...
    for idx, variant in enumerate(data.get('types', [])):
        ...
        target.attrs[-1].restrictions.path.append((
            'c', id(target), 1, 1
        ))
        target.attrs[-1].index = idx
        ...
        for attr in target.attrs:
            ...
            if type_name.startswith(PLACEHOLDER_PREFIX):
                if not type_name in cls.PLACEHOLDER_CACHE:
                    cls.PLACEHOLDER_CACHE[type_name] = cls.__placeholder_class(type_name)
